...
const manifestPath = await findFile(
...
if (manifestPath) {
  ...
  const manifestPattern = forbiddenPatterns.find((candidate) =>
    manifest.includes(candidate)
  );
  if (manifestPattern) {
    throw new Error(
      `Production manifest ${manifestPath} contains ${manifestPattern}`
    );
  }
} else {
  console.warn(
    "Production manifest not found; checked final build artifacts only"
  );
}
