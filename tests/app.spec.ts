...
test("moves focus into the modal and restores it on Escape", async ({
  page,
}) => {
  await goto(page);
  ...
  await expect(
    page.getByRole("button", { name: "Close services" })
  ).toBeFocused();
...
test("preserves the page scroll position while the modal is open", async ({
  page,
}) => {
  await goto(page);
  const services = page.getByRole("button", { name: "Services" });
...
test("restores visible focus after opening from the mobile menu", async ({
  page,
}) => {
